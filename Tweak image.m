...
static BOOL kEnableBarWidth;
static CGFloat kBarWidth = 3.f;
static NSNumber *barWidth;
static CGFloat kExpandedBarWidth;
static NSNumber *expandedBarWidth;
...
+(CGFloat)indicatorDimension {
    if(kEnabled && kEnableBarWidth) {
        return (kBarWidth >= 1.f) ? kBarWidth : %orig;
        //(0 - kBarWidth >= 2) ? (0- kShadowOffsetX) : %orig;
}
return %orig;
}
...
+(CGFloat)_expandedIndicatorDimension {
 if(kEnabled && kEnableBarWidth) {
        return (kExpandedBarWidth >= 1.f) ? kExpandedBarWidth : %orig;
        //(0 - kBarWidth >= 2) ? (0- kShadowOffsetX) : %orig;
}
return %orig;
}
...
%ctor {
    ...
    if(shouldLoad) {
        ...
        //Bar width while regular scrolling
        [preferences registerBool:&kEnableBarWidth default:NO forKey:@"enableBarWidth"];
        [preferences registerObject:&barWidth default:@3.f forKey:@"barWidth"];
        kBarWidth = [[preferences objectForKey:@"barWidth"] floatValue ];
        
 [preferences registerObject:&expandedBarWidth default:@8.f forKey:@"expandedBarWidth"];
        kExpandedBarWidth = [[preferences objectForKey:@"expandedBarWidth"] floatValue ];
